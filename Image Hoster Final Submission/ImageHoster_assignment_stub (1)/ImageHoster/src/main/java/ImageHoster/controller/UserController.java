...
public class UserController {
    ...
    public String registerUser(User user, Model model) {
        ...
        isPasswordCompliant = userService.checkPassword(user); //Validating the password
        if(isPasswordCompliant == 0){
            ...
            return usercontroller.registration(model); //If password doesn't match return to registration page with the password requirement message displayed to the user.
        }
        else
            userService.registerUser(user);
        return "users/login"; //Else redirect the user to Login Page post successful registration
    }
